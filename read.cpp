    ...
    for (int block_id : request[request_id].rest)  // 获取对象在哪个块
    {
        ...
        if (!object[object_id].request[block_id].size()) // 如果这个对象在这个块上已经被完全删除了
        {
            for (int copy_id = 1; copy_id <= 3; copy_id++)
            {
                ...
                int ptr_id = object[object_id].ptr_id[copy_id];
                int unit_id = object[object_id].unit[copy_id][block_id];
                if (disk_vector[ptr_id][disk_id].count(unit_id))
                    disk_vector[ptr_id][disk_id].erase(unit_id);
            }
        }
    }
}
...
int cal_min_dist(set<int> disk_vector[], int ptr[], int disk_id, int to)
{
...
int cal_to_pos(set<int> disk_vector[], int disk_id, int pos)
{
...
int cal_min_near_dist(set<int> disk_vector[], int disk_id, int pos)
{
    //**计算 指针以及第disk_id个磁盘中所有待读单元 与to单元的最短距离（不特指顺序）
    int resL = min(cal_min_dist(disk_vector, ptr[0], disk_id, pos), cal_min_dist(disk_vector, ptr[1], disk_id, pos));
    int resR = cal_to_pos(disk_vector, disk_id, pos) - pos;
    if (resR < 0)
        resR += V;
    return min(resL, resR);
}

int cal_weight(set<int> disk_vector[], int disk_id, int pos) // test
{
    ...
    return -cal_min_near_dist(disk_vector, disk_id, pos) * weight_to_choose_disk[0] - disk_vector[disk_id].size() * weight_to_choose_disk[1];
};
        ...
        for (int k = 1; k <= object[object_id].size; k++)
        {
            ...
            for (int j = 1; j <= 3; j++)
                for (int j2 = 0; j2 <= 1; j2++)
                {
                    ...
                    long double mark1 = cal_weight(disk_vector[j2], mn, to1), mark2 = cal_weight(disk_vector[j2],now, to2);

                    if (mark1 < mark2)
                        d = j,d2=j2; //**按最短距离判断磁盘优劣
                }
            int mn = object[object_id].replica[d];                // 放在哪个磁盘
            disk_vector[d2][mn].insert(object[object_id].unit[d][k]); // 待处理单元放入磁盘容器
            object[object_id].request[k].insert({request_id, k}); // 这个vec存储该对象的第i个块与哪些请求相关，存的值是request_id
            auto calDist = [&](int ptr, int pos)
            { return (pos - ptr - 1 + V) % V; };
            // cerr<<"C="<<C<<" D="<<D<<"\n";
            auto checkIfAbort = [&]()
            {
                if (disk_vector[d2][mn].size() < 800)
                    return false; // 如果不繁忙 很有可能能满足 所以不要废弃
                int pos = object[object_id].unit[d][k];
                int distance0 = calDist(ptr[0][mn], pos);
                int distance1 = calDist(ptr[1][mn], pos);
                if (min(distance0, distance1) > G * 35.2)
                    return true; // pass也很难pass到
                return false;
            };
...
void read(set<int>disk_vector[],int diskId, int ptr[], int last_time[], vector<int> &finish) // 选择好第几根针 就能保持原来的逻辑
{
    ...
    int to = cal_to_pos(disk_vector,diskId, ptr[diskId] + 1); //**读取顺序策略是不管进入容器顺序，优先读取距离最近的
    ...
    for (int j = 0; j < 8; j++)
        pass_read_dp[0][j] = pair<int, string>(0, "");
    ...
    for (int j = 0; j < 70; j++)                                   // 最多读1000/16个单元，1000是G的最大值
    {
        ...
        for (int k = 0; k < 8; k++)
            best_option = max(best_option, pass_read_dp[j + 1][k]);
        ...
        if (best_option.first >= 0) // 可以到达当前
        {
            ...
            for (auto [request_id, uid] : object[disk[diskId][to]].request[disk_uid[diskId][to]]) // 更新相关请求
            {
                ...
                if (!request[request_id].rest.size()) //**该请求被完成
                {
                    ...
                    vector<int> void_vec;
                    quitRequest(request_id, void_vec);
                }
            }
        ...
        else
        {
            ...
            while (best_option.first-- && ptr[diskId] + 1 != to)
                ptr[diskId] = (ptr[diskId] + 1) % V, res += 'p', last_time[diskId] = 0;
...
void read_action()
{
    ...
    for (int i = 1; i <= N; i++)
    {
        ...
        // else
        {
            read(disk_vector[0],i, ptr[0], last_time[0], finish);
            read(disk_vector[1],i, ptr[1], last_time[1], finish);
        }
    }
