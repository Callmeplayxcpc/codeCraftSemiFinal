...
{
    vector<int> tmp_disk_vector[MAX_DISK_NUM];
    for (int i = 1; i <= N; i++)
...
void read(int diskId, int ptr[], int last_time[], vector<int> &finish) // 选择好第几根针 就能保持原来的逻辑
{
    ...
    if (!disk_vector[diskId].size())
    {
        ...
        // for (int i = 1; i <= 8; i++)
        // {
        //     if (last_time[diskId] == 7)
        //         break;
        //     if (read_time[last_time[diskId]] > rest_time)
        //         break;
        //     rest_time -= read_time[last_time[diskId]];
        //     last_time[diskId] = min(7, last_time[diskId] + 1);
        //     res += 'r';
...
void divide_action()
{
    for (int i = 1; i <= N; i++)
        disk_vector[i].clear();
    for (int _ti = 0; _ti < EXTRA_TIME; _ti++)
    {
        for (int request_id : out_time_request[(_ti + timestamp) % EXTRA_TIME])
        {
            int object_id = request[request_id].object_id;
            if (request[request_id].is_done || object[object_id].is_delete)
                continue;
            for (int i = 1; i <= object[object_id].size; i++)
                object[object_id].request[i].clear();
        }
    }
    for (int _ti = 0; _ti < EXTRA_TIME; _ti++)
    {
        for (int request_id : out_time_request[(_ti + timestamp) % EXTRA_TIME])
        {
            ...
            for (int block_id : request[request_id].rest)
            {
                ...
                for (int j = 1; j <= 3; j++)
                {
                    ...
                    // 第d个副本是当前最优的副本，第j个副本是现在的副本
                    // mn代表第d个副本对应的磁盘编号，now代表第j个
                    int to1 = object[object_id].unit[d][block_id], to2 = object[object_id].unit[j][block_id];
                ...
                int mn = object[object_id].replica[d];                       // 放在哪个磁盘
                disk_vector[mn].insert(object[object_id].unit[d][block_id]); // 待处理单元放入磁盘容器
                object[object_id].request[block_id].insert({request_id, block_id});
            }
        }
    }
}